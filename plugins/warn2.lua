...
--endif--
  if value then
   if value == '1' then
    redis:hset(hash, result.from.id, '2')
   text = '[ '..name..' ]\n .شما به دلیل رعایت نکردن قوانین اخطار دریافت میکنید\nتعداد اخطار های شما : ۲/۴'
   elseif value == '2' then
  redis:hset(hash, result.from.id, '3')
  text = '[ '..name..' ]\n شما به دلیل رعایت نکردن قوانین اخطار دریافت میکنید.\nتعداد اخطار های شما : ۳/۴'
   elseif value == '3' then
   redis:hdel(hash, result.from.id, '0')
  text = '[ '..name..' ]\n به دلیل رعایت نکردن قوانین از گروه اخراج شد. (banned)\nتعداد اخطار ها : ۴/۴'
  local hash =  'banned:'..target
  redis:sadd(hash, result.from.id)
  chat_del_user(receiver, 'user#id'..result.from.id, ok_cb, false)
   end
  else
   redis:hset(hash, result.from.id, '1')
